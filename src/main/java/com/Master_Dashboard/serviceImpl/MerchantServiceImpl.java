		...
		try {
			...
			if ((merchantsRepository.existsByMerchantEmail(Encryption.encString(loginRequest.getEmail()))
				...
				if (merchantInfo.isPresent() && merchant.isPresent()) {
					...
					MerchantInfo _merchantInfo = merchantInfo.get();

					LOGGER.info("Merchant is loginRequest: {}",merchants.getIsMerchantActive());
					LOGGER.info("Merchant is loginRequest: {}",_merchantInfo.getIsMerchantActive());

					if (!(_merchantInfo.getIsMerchantActive().equalsIgnoreCase("Y")
							&& _merchantInfo.getIsMerchantActive().equalsIgnoreCase("Y"))){
						...
						if ( !verificationRepository.existsByMerchantIdAndVerificationType(merchantInfo.get().getMerchantId(),Encryption.encString("PAN"))
							...
							map.put("Client-Id", Encryption.decString(_merchantInfo.getClientId()));
							map.put("Client-Secret", Encryption.decString(_merchantInfo.getClientSecret()));
						...
						map.put("AaKy", verificationRepository.existsByMerchantIdAndVerificationType(merchantInfo.get().getMerchantId(),Encryption.encString("AADHAAR")));
						map.put("BaKy", verificationRepository.existsByMerchantIdAndVerificationType(merchantInfo.get().getMerchantId(),Encryption.encString("BANKACCOUNT")));
						map.put("email", Encryption.decString(merchants.getMerchantEmail()));
						map.put("mobileNumber", Encryption.decString(merchants.getMerchantPhone()));
